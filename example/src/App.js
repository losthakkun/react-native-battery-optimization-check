...
import { BatteryOptEnabled, RequestDisableOptimization, OpenOptimizationSettings } from 'react-native-battery-optimization-check';
...
export default function App() {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}> React Native Battery Optimization Check </Text>
      <Text style={styles.body}>Battery Optimization: {batteryOpt ? 'Enabled' : 'Disabled'}</Text>
      {batteryOpt && (
        <Button title="Disable Battery Optimization" onPress={_requestDisableBatteryOpt} />
      )}
      <Button title="Open Battery Optimization List" onPress={_openOptimizationSettings} />
      {OS === 'ios' && (
        <Text style={styles.body}> iOS does not support battery optimization </Text>
      )}
    </View>
  );
}
